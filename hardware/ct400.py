...
class CT400(AbstractCT400):
    ...
    def save_scan_wavelength_sync_file(self, path: str | Path) -> Path:
        """
        Saves the scan wavelength sync file using the CT400 DLL.

        Parameters
        ----------
        path : str | Path
            The desired file path for the output file.
        ...
        # Extract possibly modified path
        saved_path = path_buffer.value.decode("utf-8")
        logger.debug(f"Scan wavelength sync file saved at {saved_path}")
