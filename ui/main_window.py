...
from hardware.piezo_init_worker import PiezoInitWorker  # <-- NEW IMPORT
...
class MainWindow(QMainWindow):
    ...
    def _begin_lazy_init(self):
        ...
        # --- APPLY STYLESHEET TO ALIGNMENT TAB ---
        from ui.theme import CT400_CONTROL_PANEL_STYLE
    ...
    def _init_instruments(self):
        ...
        This method finds the required DLL, creates a CT400 instance, and
        handles any errors during the process by falling back to a dummy
        implementation.
        """
    ...
    @Slot(object, object)
    def _on_piezos_initialized(self, piezo_left: PiezoController, piezo_right: PiezoController):
        logger.info("MainWindow: Piezo controllers are online. Activating alignment panel.")
        self.piezo_left = piezo_left
        self.piezo_right = piezo_right

        # Update the menu actions
        self.piezo_connect_left_action.setEnabled(True)
        self.piezo_connect_left_action.setText("Disconnect Left Piezo")
        self.piezo_connect_right_action.setEnabled(True)
        self.piezo_connect_right_action.setText("Disconnect Right Piezo")

        # Pass the live hardware to the panel and activate it
        if self.ct400_device:
            self.alignment_tab.set_hardware(self.ct400_device, self.piezo_left, self.piezo_right)
        else:
            logger.error("Piezos initialized, but CT400 device is not ready. Cannot activate alignment panel.")
            self._on_piezo_init_failed("CT400 device was not available when Piezos connected.")
