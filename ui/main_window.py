...
class MainWindow(QMainWindow):
    ...
    def toggle_cinema_mode(self):
        ...
        is_visible = self.control_container.isVisible()
        self.control_container.setVisible(not is_visible)

        # Optional: Hide the status bar too for a cleaner look
    ...
    def _create_menus(self):
        ...
        dark_mode_action = QAction("Dark Mode (Lab View)", self, checkable=True)
    ...
    def _toggle_dark_mode(self, checked: bool):
        from ui.theme import _theme, get_app_stylesheet, get_camera_style, get_ct400_style

        # 1. Switch Theme State
        _theme.set_dark_mode(checked)

        # 2. Regenerate Stylesheets
        app_qss = get_app_stylesheet()
        cam_qss = get_camera_style()
        ct400_qss = get_ct400_style()

        # 3. Apply to Application (Global)
        QApplication.instance().setStyleSheet(app_qss)

        # 4. Re-apply to specific custom widgets
        self.control_panel.setStyleSheet(ct400_qss)
        self.histogram_control.setStyleSheet(ct400_qss)
        self.alignment_tab.setStyleSheet(ct400_qss)
        for panel in self.camera_panels.values():
            panel.setStyleSheet(cam_qss)

        # 5. Handle PyQtGraph (Non-QSS)
        bg_color = "#1e1e1e" if checked else "w"
        axis_color = "#e5e7eb" if checked else "k"

        # Update Plot Widgets
        for widget in [self.plot_widget.plot_widget, self.histogram_widget.plot_widget]:
            widget.setBackground(bg_color)
            widget.getAxis("left").setPen(axis_color)
            widget.getAxis("bottom").setPen(axis_color)
            widget.getAxis("left").setTextPen(axis_color)
            widget.getAxis("bottom").setTextPen(axis_color)
            # Update grid alpha for better visibility in dark mode
            grid_alpha = 0.15 if checked else 0.3
            widget.plotItem.ctrl.gridAlpha = grid_alpha  # Internal hack or re-call showGrid
            widget.showGrid(x=True, y=True, alpha=grid_alpha)

        logger.info(f"Switched to {'Dark' if checked else 'Light'} mode.")
